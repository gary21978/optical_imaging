...
class Mask:
    ...
    def __init__(self, geometry: torch.Tensor=None, pixelSize: int=25, device: torch.device=None):
        ...
        if (geometry is None or type(geometry) is not torch.Tensor) or (len(geometry.size()) != 2 or geometry.size()[0] != geometry.size()[1]): #First, does it exist? Second, is it the right shape
            ...
            self.geometry = geometry.to(dtype=torch.complex64, device=self.device) # Eli comment: use complex type instead
    ...
    def _ffFraunhofer(self, epsilon, N: int) -> torch.Tensor: #this all comes from [1]
        ...
        usqMask = self.geometry.unsqueeze(0).unsqueeze(0).to(torch.complex64)

        scaledMaskReal = torch.nn.functional.interpolate(usqMask.real, scale_factor=epsilon, mode='bilinear').squeeze(0).squeeze(0)
        scaledMaskImag = torch.nn.functional.interpolate(usqMask.imag, scale_factor=epsilon, mode='bilinear').squeeze(0).squeeze(0)
        scaledMask = scaledMaskReal + 1j*scaledMaskImag ## Eli comment: real and imag parts
